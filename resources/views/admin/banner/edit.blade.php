        ...
        <div class="col-12">
            <div class="card shadow">
                ...
                <div class="card-header bg-transparent border-0">
                    <span class="h5">{{ __('modules.edit', ['module' => trans_choice('modules.banner', 1)]) }}</span>
                </div>

                <form action="{{ route('admin.banners.update', ['banner' => $banner->id]) }}" method="POST" role="form" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                        <div class="form-group">
                            <label for="title" class="col-form-label">{{ __('labels.title') }} <span class="text-danger">*</span></label>
                            <input type="text" id="title" name="title" value="{{ old('name', $banner->title) }}" class="form-control ucfirst @error('title') is-invalid @enderror">
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control select2 @error('status') is-invalid @enderror">
                                ...
                                <option value="{{ $status }}" {{ old('status', $banner->status) == $status ? 'selected' : null }}>{{ $display }}</option>
                                @empty
                                @endforelse
                            </select>
                            @error('status')
                        ...
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-12 col-sm-6 col-lg-3">
                                    <img src="{{ $banner->media->full_file_path ?? $default_preview }}" alt="preview" class="custom-img-preview img-thumbnail d-block mx-auto">
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea type="text" id="description" name="description" class="form-control @error('description') is-invalid @enderror" cols="100" rows="7">{{ old('description', $banner->description) }}</textarea>
                            @error('description')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
