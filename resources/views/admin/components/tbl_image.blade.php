    ...
    <table id="tbl_image" class="table w-100 table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                @if (isset($action) && $action)
                <td>
                    @include('admin.components.btn_action', ['no_action' => $image->is_thumbnail, 'download' => ['route' => $image->full_file_path,'attribute' => 'download'], 'delete' => ['permission' => 'merchant.update','route' => route('admin.media.destroy', ['media' => $image->id])]])
                </td>
                @endif
            </tr>
            @empty
